...
public class GenealogyBigTreeView extends View {
	...
	public void onTouch(MotionEvent event) {
		...
		switch(event.getAction()) {
			case MotionEvent.ACTION_DOWN:	x_down= (int)event.getX(0);	y_down= (int)event.getY(0);	  		deltaXmouv = 0;	deltaYmouv = 0;	break;
			case MotionEvent.ACTION_UP:		x_up= (int)event.getX(0);	y_up= (int)event.getY(0);											break;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i=0; i < tableau_ascendants.size(); i++) {
			...
			if (personne.isValid()){tableau_coord_horizontales_noms.add(coord_horizontale_milieu_nom);}
